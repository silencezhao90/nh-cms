...
<script>
	export default {
		...
		methods: {
			...
			//加载树数据
			async getGroup(){
				var res = await this.$API.admin.role.listTree.get();
			...
			submit(){
				this.$refs.dialogForm.validate(async (valid) => {
					if (valid) {
						if (this.mode == 'add') {
							this.isSaveing = true;
							var res = await this.$API.admin.role.create.post(this.form);
							...
							this.isSaveing = true;
							var updRes = await this.$API.admin.role.update.put(this.form.id, this.form);
							this.isSaveing = false;
