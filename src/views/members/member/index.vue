	...
	<el-container>
		...
		<el-main class="nopadding">
			<scTable ref="table" :apiObj="list.apiObj" row-key="id" @selection-change="selectionChange" stripe>
				...
				<el-table-column label="推荐人" prop="referee_name" width="180"></el-table-column>
				<el-table-column label="头像" width="100">
					<template #default="scope">
						<el-avatar :src="scope.row.avatar" size="large" shape="square"></el-avatar>
					</template>
				</el-table-column>
				<el-table-column label="等级" prop="grade_name" width="100"></el-table-column>
				...
				<el-table-column label="状态" prop="status" width="100" sortable='custom' column-key="filterType" :filters="[{text: '正常', value: 1}, {text: '拉黑', value: 0}]">
					<template #default="scope">
						<el-switch v-model="scope.row.status" @change="statusChange(scope.row)" />
					</template>
				</el-table-column>
				<el-table-column label="客户端" prop="client" width="180"></el-table-column>
				...
				<el-table-column label="注册时间" prop="register_time" width="200"></el-table-column>
				<!-- <el-table-column label="删除" prop="title" width="180"></el-table-column> -->
	...
	export default {
		...
		data() {
			return {
				...
				list: {
					apiObj: this.$API.member.member.list
				},
				selection: []
			}
		},
		...
		methods: {
			...
			async table_del(row, index){
				...
				var res = await this.$API.content.channel.delete(reqData);
				if(res.code == 20000){
